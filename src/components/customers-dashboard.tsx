  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const filtered = (customers ?? []).filter((customer: Customer) =>
        [customer.customer_code, customer.client_name, customer.country, customer.inco_terms]
          .filter(Boolean)
          .join(' ')
          .toLowerCase()
          .includes(needle)
            ...
            <div className="p-4 bg-muted rounded-lg">
              {searchResults.length > 0 ? (
                <div className="space-y-3">
                  <p className="text-sm font-semibold">{language === 'ar' ? 'نتائج العملاء الموثقة' : 'Verified customer matches'}</p>
                  <Table>
                    <TableHeader>
                      <TableRow>
                        <TableHead>{t.tableHeaderCustomerCode}</TableHead>
                ...
                <p className="text-sm text-muted-foreground">{language === 'ar' ? 'لا توجد نتائج موثقة مطابقة لهذا البحث.' : 'No verified customer matches were found for this search.'}</p>
              )}
            </div>
          )}
        </CardContent>
        ...
        <CardContent className="space-y-4">
          ...
          <form onSubmit={handleChatSubmit} className="flex gap-2">
            <Input 
              ...
              placeholder={language === 'ar' ? 'اسأل عن عميل أو وجهة أو منتج' : 'Search verified customers...'}
              className="flex-1"
            />
            <Button type="submit" disabled={isChatLoading}>
              {isChatLoading ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : (language === 'ar' ? 'إرسال' : 'Send')}
            </Button>
          </form>
        </CardContent>
      </Card>
