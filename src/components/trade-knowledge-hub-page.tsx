...
import { ExternalLink, Brain, Layers3, ShieldCheck, Search, Sparkles, TrendingUp, FilterX } from 'lucide-react';
...
export function TradeKnowledgeHubPage() {
  ...
  const insights = useMemo(() => {
    const briefQuery = [selectedCategory?.name ?? '', query].filter(Boolean).join(' ').trim();
    const ranked = searchTradeInsights(briefQuery || '', 24);
    if (!selectedCategory) return ranked.slice(0, 12);
    return ranked.filter((item) => siteMatchesCategorySlug(item.main_category, selectedCategory.slug)).slice(0, 12);
